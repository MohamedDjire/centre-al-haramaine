...
function chmc_save_upload(array $file, string $prefix, string $uploadDir, int $maxBytes, array $allowedMime): string
{
...
try {
    $docActe = chmc_save_upload($_FILES['acte_naissance'], 'acte', $uploadDir, $maxBytes, $allowedDocs);
    $docBulletin = chmc_save_upload($_FILES['bulletin'], 'bulletin', $uploadDir, $maxBytes, $allowedDocs);
    $docPhoto = chmc_save_upload($_FILES['photo'], 'photo', $uploadDir, $maxBytes, $allowedImages);
...
$stmt = $conn->prepare(
    "INSERT INTO `{$tInscriptions}` (nom, prenom, sexe, date_naissance, niveau, parent_nom, telephone, whatsapp, adresse, document, document_bulletin, document_photo)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
);
$stmt->bind_param(
    'ssssssssssss',
...
if (!$stmt->execute()) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'message' => 'Erreur insertion']);
    $stmt->close();
    exit;
}
...
echo json_encode(['ok' => true, 'message' => 'Inscription enregistree.']);
